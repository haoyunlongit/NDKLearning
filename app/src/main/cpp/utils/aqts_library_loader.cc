...
namespace aqts {
    ...
    jint AqtsOnLoad(JavaVM *vm, void *reserved) {
        ...
        Logger::debug("stevenhao", "runtime_instance_ %p", runtime_instance_);
        ...
        Logger::debug("stevenhao", "vm_ext %d", offset);
...
//            Logger::debug("stevenhao", "monitor_timeout_ns_ %d", tartget->monitor_timeout_ns_);
//            Logger::debug("stevenhao", "implicit_null_checks_ %d", tartget->implicit_null_checks_);
//            Logger::debug("stevenhao", "implicit_so_checks_ %d", tartget->implicit_so_checks_);
//            Logger::debug("stevenhao", "implicit_suspend_checks_ %d", tartget->implicit_suspend_checks_);
//            Logger::debug("stevenhao", "no_sig_chain_ %d", tartget->no_sig_chain_);
//            Logger::debug("stevenhao", "force_native_bridge_ %d", tartget->force_native_bridge_);
//            Logger::debug("stevenhao", "is_native_bridge_loaded_ %d", tartget->is_native_bridge_loaded_);
//            Logger::debug("stevenhao", "is_native_debuggable_ %d", tartget->is_native_debuggable_);
//            Logger::debug("stevenhao", "async_exceptions_thrown_ %d", tartget->async_exceptions_thrown_);
//            Logger::debug("stevenhao", "non_standard_exits_enabled_ %d", tartget->non_standard_exits_enabled_);
//            Logger::debug("stevenhao", "is_java_debuggable_ %d", tartget->is_java_debuggable_);
    ...
    void* getRuntime() {
        ...
        if (handle == NULL) {
            ...
            if (error != NULL) {
                Logger::debug("stevenhao", "Failed to open libart.so: %s", error);
            } else {
                Logger::debug("stevenhao", "Failed to open libart.so");
            }
        }
        Logger::debug("stevenhao", "handle point %p", handle);
    ...
    static void print_data(void* _base, size_t _size) {
        for(auto i = 0; i < _size; i += 16) {
            ...
            Logger::debug("stevenhao", "%p: %s", current, buffer);
        }
    }
